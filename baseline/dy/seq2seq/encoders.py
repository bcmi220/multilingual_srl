...
class TransformerEncoderWrapper(EncoderBase):
    def __init__(self, d_input, pc, d_model=None, d_ff=None, num_heads=4, layers=1, dropout=0.5, name='transformer-encoder-wrapper', **kwargs):
        pc = pc.add_subcollection(name=name)
        super(TransformerEncoderWrapper, self).__init__(pc)
        if d_model is None:
            d_model = d_input
        self.proj = Linear(d_model, d_input, pc) if d_input != d_model else lambda x: x
        self.transformer = TransformerEncoderStack(num_heads, d_model=d_model, pc=pc, pdrop=dropout, scale=True, layers=layers, d_ff=d_ff)
