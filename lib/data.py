...
class Vocab(object):
	...
	def __init__(self, input_file, pret_file, min_occur_count=2):
		...
		self._id2word = ['<PAD>','<UNK>','<DUMMY>']
		self._id2lemma = ['<PAD>','<UNK>','<DUMMY>']
		self._id2tag = ['<PAD>','<UNK>','<DUMMY>']
		self._id2rel = ['<PAD>']
	...
	def rel2id(self, xs):
		if isinstance(xs, list):
			return [self._rel2id[x].get(x, self.UNK) for x in xs]
...
class DataLoader(object):
	...
	def get_batches(self, batch_size, shuffle= True):
		...
		for bkt_idx, bkt_batch in batches:
			...
			syn_masks = self._buckets[bkt_idx][:,bkt_batch, 5]
			seq_lens = np.array(self._buckets_lens[bkt_idx][bkt_batch])
			yield word_inputs, lemma_inputs, tag_inputs, arc_targets, rel_targets, syn_masks, seq_lens
